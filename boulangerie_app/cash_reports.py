...
def _cash_chart(rows: list[dict[str, Any]]) -> Drawing:
    ...
    chart.data = [
        ...
        [float(row.get("Solde", 0) or 0) for row in rows],
    ]
    ...
    chart.bars[2].fillColor = colors.HexColor("#B30000")
...
def create_cash_balance_pdf_report(
    ...
    *,
    ...
    detail_rows = [["Date", "Reçu", "Dettes payées", "Entrées", "Dépenses", "Solde", "Dettes restantes"]]
    for row in rows:
    ...
    elements.append(_pdf_table(detail_rows, [21 * mm, 25 * mm, 27 * mm, 25 * mm, 25 * mm, 25 * mm, 28 * mm]))
    doc.build(elements)
    return report_path
...
def create_cash_balance_excel_report(
    ...
    *,
    ...
    data_start = 17
    headers = ["Date", "Bacs", "Attendu", "Reçu", "Dettes payées", "Entrées", "Dépenses", "Solde", "Solde cumulé", "Dettes restantes"]
    ...
    if rows:
        bar = BarChart()
        bar.title = "Entrées, dépenses et solde"
        bar.y_axis.title = "Montant"
        bar.x_axis.title = "Date"
        bar.add_data(Reference(sheet, min_col=6, max_col=8, min_row=data_start, max_row=last_row), titles_from_data=True)
        ...
        line.add_data(Reference(sheet, min_col=9, min_row=data_start, max_row=last_row), titles_from_data=True)
