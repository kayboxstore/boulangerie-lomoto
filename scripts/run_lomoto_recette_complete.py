...
class ApiClient:
    ...
    def request(
        ...
        request_headers = {
            ...
            "User-Agent": "LomotoRecette/1.5.3",
        }
    ...
    def text(self, path: str) -> str:
        request = urllib.request.Request(
            self.base_url + path,
            headers={"User-Agent": "LomotoRecette/1.5.3", "Cache-Control": "no-cache"},
            method="GET",
        )
        with self.opener.open(request, timeout=30) as response:
...
def fetch_json(url: str, timeout: int = 15) -> dict[str, Any]:
    request = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": "LomotoRecette/1.5.3"})
